...
def test_kp_search_by_name():
    ...
    assert resp.status_code == 200
    # {"docs":[{"id":775276,"name":"Зверополис","backdrop"releaseYears":[]}],"total":4,"limit":10,"page":1,"pages":1}
    # assert resp.json{"docs": [{"id": 775276, "name": "Зверополис"}
    # assert resp.json()["id"] == 775276
    # assert resp.json()["name"] == "Зверополис"
...
def test_kp_search_seasons():
    ...
    assert resp.status_code == 200
    # assert resp.json()["movieId"] == 571335
